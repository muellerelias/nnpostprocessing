...
def main(args):
    ...
    # convert the data
    train_dataset, train_shape = converter.convert_numpy_to_multi_input_dataset_temperature_only_date(
        train_data, batchsize=args.batchsize, shuffle=1000, shape=True)
    valid_dataset = converter.convert_numpy_to_multi_input_dataset_temperature_only_date(
        valid_data, batchsize=1, shuffle=100)

    model = modelprovider.build_multi_input_model_only_temperature(
        (1,), train_shape[2])
